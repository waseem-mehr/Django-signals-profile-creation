...
def productView(request, myid):
    ...
    profile = Profile.objects.get(user=request.user)
    comments = Commment.objects.filter(
        description=product
    )
    ...
    if request.method == 'POST':
        if 'submit_comment' in request.POST:
            ...
            if request.POST.get('comment_text').strip() != '':
                Commment.objects.create(
                    comment_text=request.POST.get('comment_text'),
                    description=product,
                    profile=profile
                )
                return redirect(f'/products/{myid}')

        else:
            product.delete()
            return redirect('profile')

    return render(request, 'user_profile/prodView.html', {'product': product, 'profile': profile, 'comments': comments})
    # product[0]
